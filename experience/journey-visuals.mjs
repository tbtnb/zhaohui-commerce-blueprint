...
export function createWorld(host,scene){
 ...
 function update(frame,{indices=[0,1,2],paused=false,variant='normal',revision=1,quiet=false,detail='',resolved=false,pendingIndices=[]}={}){
  lastFrame=frame;lastOptions={indices,paused,variant,revision,quiet,detail,resolved,pendingIndices};
  const compact=host.clientWidth<500;
  const poses=posesFor(scene,frame,compact);host.dataset.frame=String(frame);host.dataset.variant=variant;host.dataset.quiet=String(quiet);host.dataset.resolved=String(resolved);host.dataset.compact=String(compact);host.style.setProperty('--frame',Math.max(0,frame));host.classList.toggle('manual-frame',paused);decor.update(frame);
  actors.forEach(({card,value,verdict},i)=>{
   ...
   const retainOriginal=['kit','sizes','inventory','deadline','signal','freshness','damage','returns','split','measure','funnel','inspect'].includes(scene.kind);
   value.textContent=frame>=3&&picked?(retainOriginal?`${scene.before[i]} → ${scene.after[i]}`:scene.after[i]):scene.before[i];
   const pending=frame>=3&&(pendingIndices.includes(i)||(variant==='issue'&&resolved&&i===indices[0]));
   if(pending)value.textContent=scene.before[i]+' → 单独交给人确认';
   const attention=pending||/待|核|缺|先|不|暂停/.test(scene.after[i]);card.classList.toggle('attention',attention&&picked&&frame>=3);
