...
export function frameStory(scene,a,frame,{variant='normal',resolved=false,quiet=false,duplicate=false}={}){
  ...
  if(variant==='issue'&&frame>=3&&resolved)return {title:frame<4?'异常单独保留，继续整理其余事项':frame===4?scene.result:'保存结果和未解决的问题',description:'示例中把受影响的第一项列为待人工确认，不再套用原来的处理建议。其余内容继续整理，完成的是本次清单，不是替人解决了所有业务问题。',blocked:false};
  if(duplicate&&frame===1)return {title:'这是同一份输入',description:'示例记录已经处理过，查看原结果，不重新生成第二份待办。',blocked:false};
  if(duplicate&&frame>=2)return {title:'继续原来的记录',description:'本例确认是同一份输入，因此查看已保存的结果，不再生成新待办。真正的新变化仍会进入下一次检查。',blocked:false};
